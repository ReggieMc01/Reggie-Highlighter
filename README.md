...
- **AI-Powered Analysis** – Uses OpenAI's GPT-4o-mini to identify key concepts in any text
- **Smart Highlighting** – Color-coded highlights based on importance score (high / medium / low)
- **Interactive Tooltips** – Hover over highlights to see importance scores
- **Two Modes** – Analyze selected text or the entire page
- **Results Panel** – View all extracted concepts with scores and explanations in the popup
- **Privacy First** – Your API key is stored locally and only sent to OpenAI
...
- Stored in `chrome.storage.local` (never leaves your browser except to call OpenAI)
- Used with the cost-effective `gpt-4o-mini` model
- Typical cost: ~$0.001 per page analyzed
